...
export default async function NeedHypothesisDetailPage({
  ...
  return (
    <section className="page-stack">
      <Link className="back-link" href="/need-hypotheses">
        ← Need Hypotheses
      </Link>
      ...
      <div className="page-heading detail-heading">
        ...
        <div className="detail-badges">
          <span className="hypothesis-pill">
            {Math.round(hypothesis.confidence * 100)}% confidence
          </span>
          <span className={`hypothesis-pill hypothesis-urgency-${hypothesis.urgency}`}>
            {readable(hypothesis.urgency)} urgency
          </span>
          <span className="hypothesis-pill">{readable(hypothesis.status)}</span>
        </div>
      </div>
            ...
            {hypothesis.source_contributions.map((item) => (
              <article key={item.independence_key}>
                <div>
                  ...
                  <span className={`hypothesis-pill hypothesis-polarity-${item.polarity}`}>

    ...
    def __init__(
        ...
        *,
        ...
        if entry.policy.id != self.source_id:
            raise ValueError("Greenhouse adapter requires its source policy")
        if not any(board.enabled for board in boards):
            raise ValueError("Greenhouse adapter requires an enabled board")
        ...
        try:
            with httpx.Client(
                ...
                batch = collect_greenhouse_jobs(
                    GreenhouseClient(
                        http_client,
                        boards_base_url=self._entry.policy.base_url,
                    ),
                    self._entry,
                    self._boards,
...
def _checkpoint_from_payload(
    ...
    return GreenhouseCheckpoint(fingerprints)

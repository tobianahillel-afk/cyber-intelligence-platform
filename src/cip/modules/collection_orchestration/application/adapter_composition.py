...
from cip.modules.collection_orchestration.application.intelligence_registration import register_intelligence_adapters
from cip.modules.collection_orchestration.application.lever_adapter import LeverAdapter
from cip.modules.collection_orchestration.application.passive_infrastructure_registration import register_passive_infrastructure_adapters
...
from cip.modules.collection_orchestration.application.search_archive_registration import register_search_archive_adapters
from cip.modules.collection_orchestration.application.smartrecruiters_adapter import SmartRecruitersAdapter
from cip.modules.collection_orchestration.application.ted_adapter import TedSearchAdapter
from cip.modules.collection_orchestration.application.vulnerability_registration import register_vulnerability_adapters
...
def build_runtime_adapters(
    ...
    *,
    ...
    register_identity_adapters(adapters, entries_by_id, inputs.identity_targets, timeout_seconds=timeout_seconds)
    _register_public_web_adapters(adapters, entries_by_id, inputs.public_web_targets, timeout_seconds=timeout_seconds)
    register_developer_ecosystem_adapters(
        adapters,
        entries_by_id,
    ...
    register_vulnerability_adapters(adapters, entries_by_id, inputs.vulnerability_targets, timeout_seconds=timeout_seconds)
...
def _register_core_adapters(
    ...
    if greenhouse_entry is not None and any(board.enabled for board in inputs.greenhouse_boards):
        _register(adapters, GreenhouseAdapter(greenhouse_entry, inputs.greenhouse_boards, timeout_seconds=timeout_seconds))
    ...
    if lever_entry is not None and any(site.enabled for site in inputs.lever_sites):
        _register(adapters, LeverAdapter(lever_entry, inputs.lever_sites, timeout_seconds=timeout_seconds))
    smartrecruiters_entry = entries_by_id.get(SmartRecruitersAdapter.source_id)
    if smartrecruiters_entry is not None and any(company.enabled for company in inputs.smartrecruiters_companies):
        _register(adapters, SmartRecruitersAdapter(smartrecruiters_entry, inputs.smartrecruiters_companies, timeout_seconds=timeout_seconds))


def _register_if_present(
    adapters: dict[tuple[str, str], CollectionAdapter],
    entries_by_id: dict[str, SourceRegistryEntry],
    adapter_type: type[CisaKevAdapter] | type[TedSearchAdapter] | type[BoampAdapter] | type[DecpAdapter],
    timeout_seconds: float,
) -> None:
...
def _register_public_web_adapters(
    ...
    *,
    ...
    for target in targets:
        ...
        _register(adapters, PublicWebAdapter(entry, target, timeout_seconds=timeout_seconds))


def _register(adapters: dict[tuple[str, str], CollectionAdapter], adapter: CollectionAdapter) -> None:

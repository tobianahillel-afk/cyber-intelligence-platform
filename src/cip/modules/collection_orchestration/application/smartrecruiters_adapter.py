    ...
    def __init__(
        ...
        *,
        ...
        if entry.policy.id != self.source_id:
            raise ValueError("SmartRecruiters adapter requires its source policy")
        if not any(company.enabled for company in companies):
            raise ValueError("SmartRecruiters adapter requires an enabled company")
    ...
    def collect(
        ...
        *,
        ...
        try:
            with httpx.Client(
                ...
                batch = collect_smartrecruiters_jobs(
                    SmartRecruitersClient(
                        http_client,
                        companies_base_url=self._entry.policy.base_url,
                    ),
                    self._entry,
                    self._companies,
        ...
        return AdapterCollectionBatch(
            ...
            checkpoint_payload={
                "fingerprints": {
                    company_id: dict(values)
                    for company_id, values in batch.checkpoint.fingerprints.items()
                }
            },
...
def _checkpoint_from_payload(
...
) -> SmartRecruitersCheckpoint | None:
    if payload is None:
        return None
    ...
    for company_id, raw_jobs in raw_fingerprints.items():
        if not isinstance(company_id, str) or not isinstance(raw_jobs, dict):
        ...
        fingerprints[company_id] = jobs
    return SmartRecruitersCheckpoint(fingerprints)


def _invalid_checkpoint() -> AdapterExecutionError:
    return AdapterExecutionError(
        "checkpoint fingerprints must contain string company, job, and hash values",
        error_code="invalid_checkpoint",
        retryable=False,
    )

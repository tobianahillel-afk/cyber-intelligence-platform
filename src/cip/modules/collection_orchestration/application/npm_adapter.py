...
from cip.adapters.sources.developer_ecosystem.schemas import NpmPackageRecord
...
class NpmPackageAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._targets = targets_for_kind(targets, DeveloperTargetKind.NPM_PACKAGE)
    ...
    def collect(
        ...
        *,
        ...
        requested_name = target.name or ""
        source_url = (
            f"{self._entry.policy.base_url}{quote(requested_name, safe='@-_.')}"
        )
        body = fetch_json(
            self._entry,
            source_url,
        ...
        try:
            record = NpmPackageRecord.model_validate_json(body)
        except ValidationError as exc:
            raise _schema_error() from exc
        if record.name.casefold() != requested_name.casefold():
            raise _identity_error()
        canonical_url = (
            f"https://www.npmjs.com/package/{quote(record.name, safe='@/-_.')}"
        )
        return package_batch(
            ...
            PackageProjectionInput(
                ...
                title=record.name,
                excerpt=version_excerpt(record.latest_version, record.description),
...
def _identity_error() -> AdapterExecutionError:
    return AdapterExecutionError(
        "npm response does not match requested package",

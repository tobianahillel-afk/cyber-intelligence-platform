...
from cip.modules.source_portfolio.domain.models import BackfillState, FreshnessState
...
class HistoricalProjectionAdapter:
    ...
    def collect(
        ...
        *,
        ...
        context = checkpoint_payload[BACKFILL_CONTEXT_KEY]
        assert isinstance(context, dict)
        assert context["lower_bound"] == "2025-01-01"
        assert context["upper_bound"] == "2025-02-01"
        observation = RawObservation(
            ...
            source_record_key="historical-1",
            source_url="https://example.invalid/records/historical-1",
            payload_hash_sha256="1" * 64,
            data_categories=frozenset({self.data_category}),
            collected_at=collected_at,
            source_updated_at=collected_at - timedelta(days=30),
            schema_fingerprint="reference-v1",
            retention_until=retention_until,
        )
        next_checkpoint = dict(checkpoint_payload)
        next_checkpoint["sequence"] = 1
        return AdapterCollectionBatch(
            observations=(observation,),
            checkpoint_payload=next_checkpoint,
            not_modified=False,
            commercial_projections=(cast(CommercialProjection, object()),),
            identity_projections=(cast(IdentityProjection, object()),),
        )


def test_raw_only_backfill_worker_persists_observation_without_signals() -> None:
    factory = _factory()
    adapter = HistoricalProjectionAdapter()
    ...
    with factory() as session:
        ...
        assert session.scalar(select(func.count(CommercialSignalRecord.id))) == 0
        partition = session.get(BackfillPartitionRecord, partition_id)
        assert partition is not None
        assert partition.state == BackfillState.COMPLETED.value
        assert partition.cursor == {"sequence": 1}
    ...
    assert idle.status is BackfillWorkerStatus.IDLE

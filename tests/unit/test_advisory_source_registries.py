...
SOURCE_PATH = Path("policies/sources.advisories.yml")
PORTFOLIO_PATH = Path("policies/source_portfolio.advisories.yml")
EXPECTED_IDS = {
...
def test_advisory_sources_are_governed_but_not_authorized() -> None:
    ...
    assert all(
        DataCategory.VULNERABILITY_METADATA in entry.policy.allowed_data_categories
        for entry in entries
    )


def test_advisory_portfolio_entries_are_non_executable_candidates() -> None:
    ...
    assert all(entry.metadata.get("credential_use") == "forbidden" for entry in entries)
    assert all(
        entry.metadata.get("exposure_verification") == "forbidden"
        for entry in entries
    )
    assert all(
        entry.metadata.get("automatic_opportunity") == "forbidden"
        for entry in entries
    )
    assert all(

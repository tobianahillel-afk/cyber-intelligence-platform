...
MODULE_ROOT = Path("src/cip/modules/professional_context")
...
_FORBIDDEN_DOMAIN_IMPORTS = (
    "fastapi",
    "sqlalchemy",
    "cip.modules.professional_context.api",
    "cip.modules.professional_context.infrastructure",
)


def test_professional_context_cannot_collect_or_trigger_commercial_actions() -> None:
    violations = _find_forbidden_imports(MODULE_ROOT, _FORBIDDEN_MODULE_IMPORTS)

    assert violations == []


def test_professional_context_domain_is_framework_and_infrastructure_free() -> None:
    violations = _find_forbidden_imports(DOMAIN_ROOT, _FORBIDDEN_DOMAIN_IMPORTS)

    assert violations == []


def test_professional_context_has_no_browser_or_platform_automation_runtime() -> None:
    forbidden_tokens = (
        "webdriver",
        "browser.new_page",
        "page.goto(",

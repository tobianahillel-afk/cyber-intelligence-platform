...
def main() -> None:
    ...
    organization = _organization(now)
    target, browser_entry = _governed_browser_target(organization, fixture_url, now)
    ...
    network_count, script_count = _assert_capture(states)
...
xhr.open("GET", "/get?source=sa16-l11");
xhr.send();
...
def _assert_capture(states) -> tuple[int, int]:
    ...
    if not any(url.startswith(f"{_ORIGIN}json") for url in source_urls):
        raise RuntimeError("SA16-L11 did not capture the same-origin fetch JSON response")
    if not any(url.startswith(f"{_ORIGIN}get?") for url in source_urls):
